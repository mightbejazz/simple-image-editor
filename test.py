...
def main():
    ...
    class Editor(ctk.CTkFrame):
        def __init__(self, master):
            ...
            self.configure(width=app_width-frwidth, height=master.winfo_height())
    ...
    class App(ctk.CTk):
        def __init__(self):
            ...
            xtop = (screen_width - app_width) / 2
            ytop = (screen_height - app_height) / 2
            
            ctk.set_appearance_mode("dark")
            ctk.set_default_color_theme("blue")
            self.title("Image Processing")
            self.geometry(f"{app_width}x{app_height}+{int(xtop)-100}+{int(ytop)-100}")
